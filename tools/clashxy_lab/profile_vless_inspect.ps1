...
try {
    ...
    $model = & (Join-Path $PSScriptRoot 'extract_vless_reality.ps1') -Inbound $inbound -Client $client
    $endpointSummaries = @(
        foreach ($endpoint in @($model.Endpoints)) {
            [pscustomobject][ordered]@{
                ...
                RealityPublicKeyPresent = -not [string]::IsNullOrWhiteSpace([string]$endpoint.RealityPublicKey)
                RealityShortIdPresent   = -not [string]::IsNullOrWhiteSpace([string]$endpoint.RealityShortId)
            }
        }
    )
